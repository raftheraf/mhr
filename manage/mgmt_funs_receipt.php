...
function show_receipt($id){
	...
	echo '<table><tr><td>';
	echo '<fieldset>';
	echo '<legend><b>'.$description.'</b></legend>';
	echo '';
	echo '<p> Tavolo: <b>'.$tavolo_numero.'</b></p>';
	echo '<table align=center width=300px bgcolor="'.color(-1).'">';
	echo '<thead ><tr>
	<td align=center><b>'.ucfirst(phr('QUANTITY')).'</b></td>
	<td align=center><b>'.ucfirst(phr('DESCRIPTION')).'</b></td>
	<td align=center><b>'.ucfirst(phr('AMOUNT')).'</b></td>
	</tr></thead>
	<tbody>';
	...
	while($row=mysql_fetch_array($res)){
		...
		//if ($tipo_corrispettivo==1) $scritta_corrispettivo_non_pagato='<b>CORRISPETTIVO NON PAGATO</b>';
		//if ($tipo_corrispettivo==0) $scritta_corrispettivo_non_pagato='CORRISPETTIVO PAGATO';
		if($tipo_corrispettivo=="T1") $descrizione_tipo_corrispettivo='In contanti';
		if($tipo_corrispettivo=="T3") $descrizione_tipo_corrispettivo='Carte di credito';
		if($tipo_corrispettivo=="T2") $descrizione_tipo_corrispettivo='Assegno';
		if($tipo_corrispettivo=="T4") $descrizione_tipo_corrispettivo='Corrispettivo non pagato';
		if($tipo_corrispettivo=="T5") $descrizione_tipo_corrispettivo='Altri metodi';

		if ($type==4) $type='Ricevuta';
		if ($type==3) $type='Fattura';
		if ($type==5) $type='Scontrino';
	...
	<td></td>
	<td align=right><b>'.ucfirst(phr('TOTAL')).'</b></td>
	<td align="right"><b>',sprintf("%0.2f",$total),'</b></td>
	</tr>';
	echo '</tbody></table>';

	echo '<br>';

	echo $type.' '.$internal_id.' del '.$date.' <br><br>';

	echo 'Tipo di Corrispettivo: ';
	echo '<b>'.$descrizione_tipo_corrispettivo."</b> - ".$tipo_corrispettivo;
	echo '<br><br>';
	echo 'Cliente: ';
	echo '<b>'.$takeaway_surname.'</b> ( id: <b>'.$customer_id.'</b> )';
	echo '<br><br>';
	echo '';
	echo "</fieldset>";
	echo '<br>';
	...
	if($tipo_corrispettivo=='T4') $selected_T4='selected';;
	if($tipo_corrispettivo=='T5') $selected_T5='selected';;

	echo '

	<fieldset>
	<legend><i>Cambia il valore del parametro nel database</i></legend>
		<div>
			<form name="form2" action="receipt.php" method="post">
				<input type="hidden" name="command" value="change_value">
				<input type="hidden" name="id" value="'.$id.'">

				<select name="tipo_corrispettivo">
						<option value="T1" '.$selected_T1.' >PAGATO CONTANTI - T1</option>
						<option value="T2" '.$selected_T2.' >PAGATO ASSEGNI - T2</option>
						<option value="T3" '.$selected_T3.' >PAGATO CARTE BANCOMAT - T3</option>
						<option value="T4" '.$selected_T4.' >NON PAGATO - T4</option>
						<option value="T5" '.$selected_T5.' >ALTRI METODI - T5</option>
					<input type="submit" value="CAMBIA">
				</select>
			</form>

	<p>
	<p style="font-family:georgia;font-size:14px;;left">
	Imposta i seguenti valori<br><br>
	PAGATO CONTANTI - T1<br>
	PAGATO ASSEGNI - T2<br>
	PAGATO CARTE BANCOMAT - T3<br>
	NON PAGATO - T4<br>
	ALTRI METODI - T5<br>
	<br>
	serve per facilitare la ricerca dei pagamenti in sospeso.<br>
	...
	echo '<fieldset>
			<legend><i>Operazioni sul documento</i></legend>';
	echo '<table width="100%"><tr>';
	echo '<td align=center>
					<p style="font-family:georgia;color:red;font-size:10px;">
						Cancella la ricevuta <br>
						dal database <br>
						nessun dato rimane<br>
...
function cambia_tipo_corrispettivo($id, $tipo_corrispettivo){
	require("./mgmt_start.php");

	$table=$GLOBALS['table_prefix'].'account_mgmt_main';

	if ($tipo_corrispettivo=='T1'){ $query="UPDATE $table SET `tipo_corrispettivo`='T1' WHERE `id`='$id'"; }
	if ($tipo_corrispettivo=='T2'){ $query="UPDATE $table SET `tipo_corrispettivo`='T2' WHERE `id`='$id'"; }
	if ($tipo_corrispettivo=='T3'){ $query="UPDATE $table SET `tipo_corrispettivo`='T3' WHERE `id`='$id'"; }
	if ($tipo_corrispettivo=='T4'){ $query="UPDATE $table SET `tipo_corrispettivo`='T4' WHERE `id`='$id'"; }
	if ($tipo_corrispettivo=='T5'){ $query="UPDATE $table SET `tipo_corrispettivo`='T5' WHERE `id`='$id'"; }

$res = mysql_db_query ($_SESSION['mgmt_db'],$query);

//redirect(basename($_SERVER["HTTP_REFERER"]),0);
}
