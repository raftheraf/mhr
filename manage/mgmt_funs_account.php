...
function account_movement_from_manage($mgmt_id){
	...
	$data['date']['month']=substr($arr['date'],5,2);
	$data['date']['day']=substr($arr['date'],8,2);
	$hour=substr($arr['date'],11,2);
	$minute=substr($arr['date'],14,2);
	$second=substr($arr['date'],17,2);
...
function account_movement_form($id=0){
	...
	if($id) {
		...
		$month=substr($arr['timestamp'],5,2);
		$day=substr($arr['timestamp'],8,2);
		$hour=substr($arr['timestamp'],11,2);
		$minute=substr($arr['timestamp'],14,2);
		$second=substr($arr['timestamp'],17,2);
	} else {
		$day=date("d",time());
		$month=date("m",time());
		$year=date("Y",time());
