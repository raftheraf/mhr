# Gestisci Spostamento Tavoli — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Creare una pagina admin `waiter/table_move_admin.php` accessibile da `tables.php` che permette a cassiere/admin di spostare o scambiare due tavoli scegliendo sorgente e destinazione da due select, con rilevamento automatico dell'operazione.

**Architecture:** Pagina PHP con doppia modalità GET (form) / POST (esecuzione). Riusa i metodi `move()` e `swap()` già presenti nella classe `table`. Il pulsante di accesso è agganciato alla variabile template `{barra_booking}` in `tables.php`.

**Tech Stack:** PHP 5.5, `mysql_*`, template `tables` esistente, JavaScript inline.
...
| File | Ruolo |
|---|---|
| `waiter/table_move_admin.php` | **Nuovo** — form + esecuzione POST |
| `waiter/tables.php` | Aggiunge pulsante di accesso dopo `barra_booking` |

---

## Task 1: Crea `waiter/table_move_admin.php`

**File:** Create `waiter/table_move_admin.php`

- [ ] **Step 1: Creare il file con il codice completo**

Creare `waiter/table_move_admin.php` con il seguente contenuto:
...
```bash
php -l waiter/table_move_admin.php
```
Risultato atteso: `No syntax errors detected in waiter/table_move_admin.php`

- [ ] **Step 3: Commit**

```bash
git add waiter/table_move_admin.php
git commit -m "feat: aggiungi pagina admin gestisci spostamento tavoli"
```

---

## Task 2: Aggiungi pulsante in `waiter/tables.php`

**File:** Modify `waiter/tables.php:42`

Il file `waiter/tables.php` ha questa riga alla linea 42:
```php
$tpl -> append ('barra_booking',barra_booking());
```

- [ ] **Step 1: Aggiungere il pulsante subito dopo la riga 42**

Trovare il blocco (riga 42):
```php
$tpl -> append ('barra_booking',barra_booking());
```
...
```bash
php -l waiter/tables.php
```
Risultato atteso: `No syntax errors detected in waiter/tables.php`

- [ ] **Step 3: Commit**

```bash
git add waiter/tables.php
git commit -m "feat: aggiungi pulsante GESTISCI SPOSTAMENTO TAVOLI in tables.php"
```
...
- [ ] Aprire `tables.php` come cassiere — verificare che il pulsante "GESTISCI SPOSTAMENTO TAVOLI" appaia sotto la riga `<<--Tavoli | Prenotazioni-->>`
- [ ] Aprire `tables.php` come cameriere semplice — verificare che il pulsante NON appaia
- [ ] Cliccare il pulsante — verificare che si apra `table_move_admin.php` con i due select
- [ ] Select A deve mostrare solo tavoli occupati/sospesi (non liberi)
- [ ] Select B deve mostrare tutti i tavoli
- [ ] Cambiando select A, verificare che il tavolo selezionato in A venga disabilitato in select B
- [ ] Scegliere Da=tavolo occupato, A=tavolo libero → ESEGUI → confirm dice "spostando →" → confermare → verifica spostamento in `tables.php`
- [ ] Scegliere Da=tavolo occupato, A=tavolo occupato → ESEGUI → confirm dice "scambiando ↔" → confermare → verifica scambio in `tables.php`
