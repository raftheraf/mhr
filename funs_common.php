...
function show_logo () {
	$ret = '<img src="'.ROOTDIR.'/images/mhr_logo.jpg" alt="RTR Restaurant" width="279" height="100">'."\n";
	return $ret;
}
...
function head_line ($title) {
	global $tpl;
//modifica 2020-05-08
//tolta la riga <script type="text/javascript" language="JavaScript" src="'.ROOTDIR.'/overlib/overlib.js"><!-- overLIB (c) Erik Bosrup --></script>
// per riattivare verificare se presente "$tmp .= '<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000;"></div>';" sotto
// vicino a $tpl -> assign("head", $output);
	$output='
	<!-- function head_line -->
	<meta name="viewport" content = "width = device-width, initial-scale = 1.0, minimum-scale = 1, maximum-scale = 1, user-scalable = no" />
	<!-- App name -->
	...
	$tmp = '<div id="myDIV"><img id="myDIVimg" alt="Loading" src="../images/loading.gif"></div>';
	//la riga sotto serve per il funzionamento della libreria overlib.js ormai in superata
...
function disconnect_line () {
	if(isset($_SESSION['userid'])) {
		...
		$output = 'Utente: '.$user->data['name'].'
							 <a href="disconnect.php"><b>LOGOUT</b></a>
							'."\n";
	}
	return $output;
}
...
//	unset($_SESSION['codice_lotteria']);
	unset($_SESSION['pagato_altri_metodi']);
	unset($_SESSION['pagato_carte_di_credito']);
