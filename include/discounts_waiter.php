...
function apply_discount_amount($sourceid,$amount){
	...
	*/
	...
	if( $amount>table_total_without_discount($sourceid) or $amount>bill_total_table() )
		return 6;
...
function discount_form_javascript($sourceid){
	...
	<input type="radio" name="discount_type" value="none" onclick="JavaScript:discount_switch();" '.$chk[0].' class="radio">'.ucfirst(phr('NONE')).'<br>
	</td></tr>
	<tr>
		<td align="left">
			<input type="radio" name="discount_type" value="percent" onclick="JavaScript:discount_switch();" '.$chk[1].'  class="radio">
	'.ucfirst(phr('PERCENTUAL')).'
		<td align="left">
			<INPUT TYPE="text" name="percent" size="2" maxlength="2" value="'.$percent.'" '.$dis[1].' class="radio">%
		</td>
	</tr>
	<tr>
		<td align="left">
			<input type="radio" name="discount_type" value="amount" onclick="JavaScript:discount_switch();" '.$chk[2].' class="radio">'.ucfirst(phr('VALUE')).'
		</td>
		<td align="left">
			<INPUT TYPE="text" name="amount" size="6" maxlength="6" value="'.$amount.'" '.$dis[2].' class="radio">'.country_conf_currency (true).'
		</td>
	</table>
	<br>
	<INPUT TYPE="SUBMIT" value="'.ucfirst(phr('APPLY_DISCOUNT')).'" class="button_big"><br>
	</FORM>
	</FIELDSET>
	';
	return $output;
