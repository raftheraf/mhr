...
// _______________________________________________________________________
	function prepare_default_array ($dishid, $input_data) {
...
// _______________________________________________________________________
	function sync_suborders () {
		...
		$sync_array = array ('suspend','printed','extra_care','sourceid','quantity','priority','paid','deleted','dest_id','menu_fisso');
// RTR END
	...
	function table_row($arr){
		...
		if(!$arr['deleted'] && $arr['printed']!=NULL && CONF_TIME_SINCE_PRINTED) {
			...
			$dishname .= ' <span class="smallest">('.orders_print_elapsed_time ($ord,true).')</span>';
			unset ($ord);
		}
		...
		if ($arr['dishid']==MOD_ID){
			...
			if ($arr['operation']==1) {
				$dishname='<b>+</b><span class="modified">';
			} elseif ($arr['operation']==-1) {
				$dishname='<b>-</b><span class="modified">';
			} elseif ($arr['operation']==0) {
				$dishname='<span class="modified">';
			}
		...
		// RTR Tabella riepilogo ordini corpo
		// row begins
		$output .= '';

		// Attenzione particolare extra care
		//RTR
		if($deleted && $arr['dishid']!=MOD_ID) {
			$output .= '
		<tr bgcolor="'.COLOR_TABLE_GENERAL.'"><td height="1px" colspan="8"></td></tr>
		<tr bgcolor="'.$classpriority.'">
		<td align="center" width="20px" height="40px" >
			<s>'.$oextra.'</s>
		</td>';
		} elseif (!$deleted && $arr['dishid']!=MOD_ID) {
			$output .= '

		<tr bgcolor="'.$classtime.'"><td height="1px" colspan="8" bgcolor="'.$classtime.'"></td></tr>

		<tr bgcolor="'.$classpriority.'">
		<td align="center" width="20px" height="40px" bgcolor="'.$classextra.'">
			<div style="color:#ffffff"><b>'.$oextra.'</b></div>
			<div style=" border:0px solid black; height:10px; width:10px; background-color:'.$classtime.'; margin:0 auto; border-radius:10px; display: inline-block; "></div>
		</td>';
		} else {
			$output .= '
		<tr bgcolor="'.$classpriority.'">
		<td height="10px" bgcolor="'.$classextra.'"></td>';
		}
		...
		// Nome del Piatto e nota_ordine
		$nota_ordine=$arr['nota_ordine'];
		if($deleted) {
			$output .= '
		<td >
			<s>'.$dishname.'</s><br>
			<div class="font11px">'.$nota_ordine.'</div>
		</td>';
		} else {
			if(!$deleted
				&& $arr['printed']==NULL
		...
		//RTR stampa a video la nota_ordine sul order_list
			$output .= '
		<td style="cursor: pointer" onclick="redir(\''.$link.'\');">
			<a href="'.$link.'">'.$dishname.'</a><br>
			<div class="font11px">'.$nota_ordine.'</div>

		</td>';
			}
			// stampa a video le varianti
			elseif(!$deleted
				&& $arr['printed']==NULL
				&& $arr['dishid']==MOD_ID) {
				$link = 'orders.php?command=listmods&amp;data[id]='.$arr['associated_id'];
				$output .= '<td style="cursor: pointer" onclick="redir(\''.$link.'\');"><a href="'.$link.'">'.$dishname.'</a></td>';
			}
			elseif(!$deleted
				&& $arr['printed']!=NULL
				&& $arr['dishid']==MOD_ID) {
		...
		//RTR stampa la priorita
		if($deleted && $arr['dishid']!=MOD_ID) {
			$output .= '<td width="20px" align="center" bgcolor="'.$classpriority.'"><s>'.$arr['priority'].'</s></td>';

		} elseif (!$deleted && $arr['dishid']!=MOD_ID) {
			$output .= '<td width="20px" align="center" bgcolor="'.$classpriority.'"><b>'.$arr['priority'].'</b></td>';
		...
		elseif (!$deleted && $arr['dishid']!=MOD_ID) {
			$output .= '<td align="right">'.$arr['price'].'</td>';
		}

		else {
			$output .= '<td align="right"><span class="modified">'.$arr['price'].'</span></td>';
		}
		...
		<td ></td>';
			...
			$output .= '
		<td align="center" onclick="redir(\''.$link.'\');">
			<a href="'.$link.'"><img src="'.IMAGE_EDITDISH.'" alt="Modifica le quantita ed il turno" border=0></a>
		</td>';
		} elseif (!$deleted
			&& $arr['dishid']==MOD_ID) {
		...
		<td align="center">';
				if($arr['quantity']>1){
					...
					//La linea sotto porta ad un nuova pagina con una ulteriore conferma rallentando l'operazione
					...
					$link = 'orders.php?command=delete&amp;data[id]='.$orderid;
...
/* ultime 2 celle nel riepilogo ordine da qui  */
				$output .= '<img src="'.IMAGE_BLANK.'" width="30px" height="30px" border=0 alt="immagine trasparente">
										</td>';
				$output .= '<td align="center">';

			if( access_allowed(USER_BIT_CASHIER) && table_is_takeaway($_SESSION['sourceid']) )
				$output .= '<a href="orders.php?command=delete&amp;data[id]='.$orderid.'">
										<img src="'.IMAGE_LITTLE_TRASH.'" width="30px" height="30px" border=0 alt="elimina righa">
										</a>';
			else
				$output	.='<img src="'.IMAGE_BLANK.'" width="30px" height="30px" border=0 alt="immagine trasparente">';
/* a qui  */
