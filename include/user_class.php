...
class user extends object {
	...
	function user($id=0) {
		...
		$this->fields_show=array('id','name','language','template','password','disabled','level','preconto','preconto1','ricevuta','ricevuta1','fattura','fattura1','scontrino');
		$this->fields_names=array(	'id'=>ucphr('ID'),
								'name'=>ucphr('NAME'),
								'user_host'=>ucphr('IP Address'),
								'language'=>ucphr('Lang'),
								'template'=>ucphr('Modello'),
								'password'=>ucphr('PSW'),
								'disabled'=>ucphr('DisAbil'),
								'preconto'=>ucphr('precon'),
								'preconto1'=>ucphr('precon1'),
								'ricevuta'=>ucphr('ricev'),
								'ricevuta1'=>ucphr('ricev1'),
								'fattura'=>ucphr('fat'),
								'fattura1'=>ucphr('fat1'),
								'scontrino'=>ucphr('scontr'));
		$this->fields_width=array(	'name'=>'20%' );
		$this->allow_single_update = array ('level','disabled','preconto','preconto1','ricevuta','ricevuta1','fattura','fattura1','scontrino');
		//$this->fields_boolean=array('preconto','preconto1','ricevuta','ricevuta1','fattura','fattura1','scontrino');
		$this -> fetch_data();
		$this -> get_level ();
	}
	...
	function list_query_all () {
		...
		$query="SELECT $table.`id`,
				...
				IF($table.`ricevuta`='0','".ucphr('NO')."','".ucphr('YES')."') as `ricevuta`,
				IF($table.`ricevuta1`='0','".ucphr('NO')."','".ucphr('YES')."') as `ricevuta1`,
				IF($table.`fattura`='0','".ucphr('NO')."','".ucphr('YES')."') as `fattura`,
				IF($table.`fattura1`='0','".ucphr('NO')."','".ucphr('YES')."') as `fattura1`,
				IF($table.`scontrino`='0','".ucphr('NO')."','".ucphr('YES')."') as `scontrino`
				FROM `$table`
				WHERE $table.`deleted`='0'
				";
	...
	function html_select($who=SHOW_ALL_USERS) {
		...
		//2019-07-03 if (system::upgradeCompleted(6))
		$query .= " AND `deleted`='0'";
		...
		$output .= '			<select name="userid" size="8" class="input">'."\n";
		$i=0;
		while ($arr=mysql_fetch_array ($res)) {
			...
			elseif ($pos_only && $user->is_cassiere()) $write_line=true;
			elseif ($admin_only && $user->is_admin()) $write_line=true;
			elseif (!$admin_only && !$waiter_only && !$pos_only) $write_line=false;
	...
	function connect () {
		...
		if(isset($_REQUEST['password']) && empty($_REQUEST['password'])
		//&& (!$user->level[USER_BIT_WAITER] && !$user->level[USER_BIT_CASHIER])
		)
	...
	function disconnect_confirm () {
		...
		$tmp .= '<br><br><a href="#" onclick="javascript:history.go(-1); return false;"><h2>'.ucfirst(phr('NO')).'</h2></a><br>'."\n";
		$tpl -> assign ('logout',$tmp);
		return 0;
	}
	...
	<tr>
		...
		<td>
		<input type="text" name="data[user_host]" value="'.$arr['user_host'].'">
		</td>
	</tr>
	...
	<tr>
		...
		<fieldset>
		<legend>Stampanti associate</legend>
		<table>
		<tr>
		<td>
		<td colspan="2">
		<input type="checkbox" name="data[preconto]" value="1"';
		if($this->data['preconto']) $output .= ' checked';
		$output .= '>'.ucphr('preconto').'
		</td>
		<td colspan="2">
		<input type="checkbox" name="data[preconto1]" value="1"';
		if($this->data['preconto1']) $output .= ' checked';
		$output .= '>'.ucphr('preconto1').'
		</td>
		<td colspan="2">
		<input type="checkbox" name="data[ricevuta]" value="1"';
		if($this->data['ricevuta']) $output .= ' checked';
		$output .= '>'.ucphr('ricevuta').'
		</td>
		<td colspan="2">
		<input type="checkbox" name="data[ricevuta1]" value="1"';
		if($this->data['ricevuta1']) $output .= ' checked';
		$output .= '>'.ucphr('ricevuta1').'
		</td>
		<td colspan="2">
		<input type="checkbox" name="data[fattura]" value="1"';
		if($this->data['fattura']) $output .= ' checked';
		$output .= '>'.ucphr('fattura').'
		</td>
		<td colspan="2">
		<input type="checkbox" name="data[fattura1]" value="1"';
		if($this->data['fattura1']) $output .= ' checked';
		$output .= '>'.ucphr('fattura1').'
		</td>
		<td colspan="2">
		<input type="checkbox" name="data[scontrino]" value="1"';
		if($this->data['scontrino']) $output .= ' checked';
		$output .= '>'.ucphr('scontrino').'
		</td>
		</tr>
		</table>
		</fieldset>
