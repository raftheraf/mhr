...
class dish extends object {
	function dish($id=0) {
		...
		$this->fields_names=array(	'id'=>ucphr('ID'),
								...
								'personal_list'=>ucphr('Lista Pers'),
								'personal_list_order'=>ucphr('Ord List Pers'),
								'asporto_list'=>ucphr('Lista Asporto'),
								'asporto_list_order'=>ucphr('Ord List Aspor'),
								'menufisso'=>ucphr('Menu Fisso'),
								'dishesmenufisso'=>ucphr('Menu Fisso'),
								'salta_stampa'=>ucphr('Salta la Stampa')
								);

		$this -> title = ucphr('DISHES');
		$this->file=ROOTDIR.'/admin/admin.php';
		$this->fields_width=array('name'=>'30%','price'=>'10%');
		$this->allow_single_update = array ('autocalc','generic','visible','personal_list','asporto_list','menufisso','salta_stampa');
	...
	*/
	function search_name_rows ($search) {
		...
		//risolve il problema degli apostrofi
	...
	function list_query_all () {
		...
		$query="SELECT
				...
				IF($table.`personal_list`='0','".ucphr('NO')."','".ucphr('YES')."') as `personal_list`,
				$table.`personal_list_order` as `personal_list_order`,

				IF($table.`asporto_list`='0','".ucphr('NO')."','".ucphr('YES')."') as `asporto_list`,
				$table.`asporto_list_order` as `asporto_list_order`,

				IF($table.`autocalc`='0','".ucphr('NO')."','".ucphr('YES')."') as `autocalc`,
				IF($table.`generic`='0','".ucphr('NO')."','".ucphr('YES')."') as `generic`,
				IF($table.`menufisso`='0','".ucphr('NO')."','".ucphr('YES')."') as `menufisso`,
				IF($table.`salta_stampa`='0','".ucphr('NO')."','".ucphr('YES')."') as `salta_stampa`
	...
	//see use in
// salta_stampa
	function getsalta_stampa() {
	return $this->data['salta_stampa'];
}

// Funzione Genera Menu Fisso
	function getdishesmenufisso() {
		return $this->data['dishesmenufisso'];
	}
	function getPrice() {
	...
	function form_new ($input_data=array()) {
		...
		if(!$editing || (isset($input_data['show_names']) && $input_data['show_names'])) {
			...
			while($arr_lang=mysql_fetch_array($res_lang)) {
				if($lang_now=stristr($arr_lang[0],$GLOBALS['table_prefix'].'dishes_')) {
					...
					$col++;
					$display->rows[$row][$col]='<input type="text" size="40" name="data[dishes_'.$lang_now.']" value="'.$lang_name.'">';
					$col++;
					$row++;
					$col=0;
				}
		...
		*************************************************/
		...
		$display->widths[$row][$col]="70%";
		$col++;
		$display->rows[$row][$col]='<input type="text"  size="40" name="data[price]" value="'.$arr['price'].'">';
		$col++;
		$row++;
		$col=0;
	...
	// RTR START
		...
		if($arr['personal_list']) $checked = ' checked';
		else $checked='';
		//$display->properties[$row][$col]='colspan=2';
		$display->rows[$row][$col]=ucphr('<b>Lista Personalizzata</b> - Visualizza il piatto nella lista sulla pagina degli ordini');
		$col++;
		$display->rows[$row][$col]='<input type="checkbox" name="data[personal_list]" value="1"'.$checked.'>';
		$col++;
		$row++;
		$col=0;
		//$display->properties[$row][$col]='colspan=2';
		$display->rows[$row][$col]=ucphr('Ordine di visualizzazione nella Lista Personalizzata');
		$col++;
		$display->rows[$row][$col]='<input type="text" size="1" name="data[personal_list_order]" value="'.$arr['personal_list_order'].'">';
		$col++;
		$row++;
		$col=0;
		// Fine Funzione personal_list

		// Funzione asporto_list
		if($arr['asporto_list']) $checked = ' checked';
		else $checked='';
		...
		//$display->properties[$row][$col]='colspan=2';
		$display->rows[$row][$col]=ucphr('<b>Lista Asporto </b>- Visualizza il piatto nella lista dei tavoli Asporto sulla pagina degli ordini');
		$col++;
		$display->rows[$row][$col]='<input type="checkbox" name="data[asporto_list]" value="1"'.$checked.'>';
		$col++;
		$row++;
		$col=0;
		//$display->properties[$row][$col]='colspan=2';
		$display->rows[$row][$col]=ucphr('Ordine di visualizzazione nella Lista Asporto');
		$col++;
		$display->rows[$row][$col]='<input type="text" size="1" name="data[asporto_list_order]" value="'.$arr['asporto_list_order'].'">';
		$col++;
		$row++;
		$col=0;
		// Fine Funzione personal_list
		...
		Menu Fisso
		*************************************************/
		if($arr['menufisso']) $checked = ' checked';
		else $checked='';
		//$display->properties[$row][$col]='colspan=2';
		...
		$display->rows[$row][$col]=ucphr('<b>Menu Fisso</b> - composto da diversi piatti con relativi turni');
		$col++;
		$display->rows[$row][$col]='<input type="checkbox" name="data[menufisso]" value="1"'.$checked.'>';
		$col++;
		$row++;
		$col=0;

		/*************************************************
		Piatti del menu fisso array dishesmenufisso
		*************************************************/
		$display->rows[$row][$col]=ucphr('Array del campo dishesmenufisso <b><i>id</i></b> spazio <b><i>priorità</i></b>: ').'';
		$col++;
		$display->rows[$row][$col]='<input type="text" size="40" name="data[dishesmenufisso]" value="'.$arr['dishesmenufisso'].'">';
		$col++;
		$row++;
		$col=0;
		...
		*************************************************/
		...
		if($arr['salta_stampa']) $checked = ' checked';
		else $checked='';
		//$display->properties[$row][$col]='colspan=2';
		...
		$display->rows[$row][$col]=ucphr('Salta la Stampa del piatto: serve per il registratore di cassa');
		$col++;
		$display->rows[$row][$col]='<input type="checkbox" name="data[salta_stampa]" value="1"'.$checked.'>';
		$col++;
		$row++;
		$col=0;
	...
	*************************************************/
	...
	$display->rows[$row][$col]="<b>".ucphr('VISIBLE_TO_WAITERS')."</b>";
	$col++;
