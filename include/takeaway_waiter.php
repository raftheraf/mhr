...
function takeaway_check_values($input_data){
	...
	empty($input_data['takeaway_month']) ||
	empty($input_data['takeaway_day'])) {
		$msg=ucfirst(phr('CHECK_DATE')).'<br>';
	}
	if(empty($input_data['takeaway_minute'])) {
		$msg=ucfirst(phr('CHECK_MINUTE')).'<br>';
	}
	if(empty($input_data['takeaway_hour'])) {
		$msg=ucfirst(phr('CHECK_HOUR')).'<br>';
	}
	...
	if($time_requested<($time_now-$time_tolerance)) {
		$msg=ucfirst(phr('CHECK_TIME_BEFORE_NOW')).'<br>';
	}
...
function takeaway_form() {
	...
	$tmp .= '<br />
	<b>Dati cliente per Asporto</b><br />
	...
	if(get_conf(__FILE__,__LINE__,'takeaway_allow_unknown_customer')) {
		$tmp .= '
			<input name="data[takeaway_surname]" type="text" value="'.$data['takeaway_surname'].'" maxlength="255" size="24">';
	} else {
		$tmp .= '
	...
	$tmp .= '
		...
		<td colspan="2" align="right">
			...
			<table>
				<tr>
					<td>
						<i>Premi il tasto verde <br />per salvare i dati</i>
					</td>
					<td>
						<input type="image" src="'.IMAGE_OK.'" alt="'.ucfirst(phr('TAKEAWAY_SUBMIT')).'" border=0 width="50px" height="50px">
					</td>
				</tr>
			</table>
