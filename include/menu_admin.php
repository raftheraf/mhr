...
class menu {
	...
	function main () {
		...
		******************************************************************************/
		...
		$this -> output .= '
			oM.level[0]=new cm_makeLevel('.$colwidth.',30,"clT","clTover",1,1,"clB",0,"bottom",0,0,0,0,0);
			oM.level[1]=new cm_makeLevel('.$colwidth.',50,"clS","clSover",1,1,"clB",0,"right",0,0,"'.ROOTDIR.'/images/menu_arrow.gif",15,15);
			oM.level[2]=new cm_makeLevel('.$colwidth.',30,"clS2","clS2over");
			oM.level[3]=new cm_makeLevel('.($colwidth-10).',30);'."\n\n";


		$index = 0;
		//Link per la pagina tavoli
		$index = $this -> tavoli($index+1,'');
		...
		/*già compreso nel menu Contabilità (accounting)
	...
	function dishes_categories($start_idx,$parent) {
		...
		//2019-07-03 if (system::upgradeCompleted(6)) $query .= " AND `deleted`='0'";
	...
	//RTR HOME PAGE
	function sospesi($start_idx,$parent) {

		$i=$start_idx;

		$this->letters[$i]=strlen(ucphr('HOME PAGE'));

		$this -> output.="\t\toM.makeMenu('m$start_idx','$parent','".ucphr('SOSPESI')."','');\n";
		$this -> links[$i]['name']=ucphr('SOSPESI').': '.ucphr('SOSPESI');
	...
	function tavoli($start_idx,$parent) {
		$i=$start_idx;

			$this -> output.="\t\toM.makeMenu('m".$i."','m".$start_idx."','"."HOME"."','".ROOTDIR."/waiter/');\n";
